...
const StegCore = (() => {
  ...
  function snapToKey(px, py, IW, IH) {
    ...
    // 1-wide interior: the row holds no key pixel, so step to the row
    // above/below, where the checkerboard parity makes this x a key pixel
  ...
  // encoded canvas can have (encodeContainer throws below it). Header length is
  // 12 + descriptor + 1 checksum, and the descriptor depends only on the effect
  // settings, so this is knowable before the canvas is sized. Deliberately a
  // slight over-estimate (widest seed, +2 slack): it's used as a floor, and a
  // canvas two pixels wider than strictly needed costs nothing.
