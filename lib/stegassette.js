...
var Stegassette = (() => {
  ...
  var CIRCLE_AREA_FACTOR = 4 / Math.PI;
  function interiorDims(dataPx, aspect, B = 0, keyless = false, fit = "compact") {
    const density = keyless ? 1 : 2;
    const capacity = fit === "circle" ? (W, H) => ellipseDataPixelCount(W, H, keyless) : keyless ? (W, H) => W * H : dataPixelCount;
    const estimate = fit === "circle" ? Math.ceil(dataPx * CIRCLE_AREA_FACTOR) : dataPx;
  ...
  function resolveBorderWidth(spec, dataPx, aspect, keyless = false, fit = "compact") {
  ...
  function autoScaleImg(img, totalBytes, B = 1, aspectOverride = null, bytesPerPixel = 3, minFullWidth = 1, keyless = false, fit = "compact") {
