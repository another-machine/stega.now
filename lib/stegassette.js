...
var Stegassette = (() => {
  ...
  __export(stegassette_exports, {
    ...
    COLLECTION_STEG: () => COLLECTION_STEG,
    COMBINE: () => COMBINE,
    ...
    TRAVERSAL_LEGACY: () => TRAVERSAL_LEGACY,
    TRAVERSAL_NAMES: () => TRAVERSAL_NAMES,
    TRAVERSAL_SHAPE: () => TRAVERSAL_SHAPE,
    ...
    ellipseDataPixelCount: () => ellipseDataPixelCount,
    ellipseFit: () => ellipseFit,
    ellipseRadius2: () => ellipseRadius2,
    encode: () => encode,
    encodeContainer: () => encodeContainer,
    encodeImageData: () => encodeImageData,
    encodeStegassette: () => encodeStegassette,
  ...
  function ellipseRadius2(x, y, W, H) {
    const nx = (x - (W - 1) / 2) / (W / 2);
    const ny = (y - (H - 1) / 2) / (H / 2);
    return nx * nx + ny * ny;
  }
  function ellipseDataPixelCount(W, H, keyless = false) {
    const inside = (x, y) => ellipseRadius2(x, y, W, H) <= 1;
  ...
  function buildDescriptor(opts) {
    ...
    if (traversal === "radial")
      s += `direction=${params.direction === "in" ? "in" : "out"}`;
  ...
  function parseRingBytes(alphas, B) {
    ...
    const params = {
      ...
      direction: d.direction === "in" ? "in" : d.direction === "out" ? "out" : void 0,
      rotation: d.rotation === "ccw" ? "ccw" : d.rotation === "cw" ? "cw" : void 0
    };
    return {
      B,
      version: recovered[4],
  ...
  function spiralPath(W, H, filter = isDataPixel, rotation = "cw") {
  ...
  function radialPath(W, H, filter = isDataPixel, direction = "out") {
    const arr = Array.from(rasterPath(W, H, filter));
    const r2 = (v) => ellipseRadius2(v % W, v / W | 0, W, H);
    arr.sort((p, q) => {
  ...
  var ellipseFit = (W, H, keyless) => ellipseDataPixelCount(W, H, keyless);
  ...
  var compactFit = (W, H, keyless) => keyless ? W * H : dataPixelCount(W, H);
  function resolveFit(fit, traversal = "raster") {
    if (fit === "compact") return compactFit;
    if (fit === "shape") return TRAVERSAL_SHAPE[traversal] ?? compactFit;
    return fit;
  }
  function areaFactor(fitFn, dataPx, aspect, keyless) {
  ...
  function interiorDims(dataPx, aspect, B = 0, keyless = false, fitFn = compactFit) {
    const density = keyless ? 1 : 2;
    const capacity = (W, H) => fitFn(W, H, keyless);
  ...
  function resolveBorderWidth(spec, dataPx, aspect, keyless = false, fitFn = compactFit) {
  ...
  function autoScaleImg(img, totalBytes, B = 1, aspectOverride = null, bytesPerPixel = 3, minFullWidth = 1, keyless = false, fitFn = compactFit) {
  ...
  function reconstructCover(image, opts) {
    ...
    if (isKeylessKeymap(opts.keymap)) {
      ...
      if (carries.some(Boolean)) {
        const pathIdx2 = getPathIndices(IW, IH, opts.traversal, opts.params, true);
        const bpp2 = opts.plan?.bytesPerPixel ?? 3;
        const nEnc2 = opts.interiorByteLength ? Math.min(pathIdx2.length, Math.ceil(opts.interiorByteLength / bpp2)) : pathIdx2.length;
        for (let pi = 0; pi < nEnc2; pi++) {
          const v = pathIdx2[pi];
  ...
  var newKey = () => toBase64(crypto.getRandomValues(new Uint8Array(32)));
  var importKey = (keyB64) => crypto.subtle.importKey("raw", fromBase64(keyB64), "AES-GCM", false, [
    "encrypt",
    "decrypt"
  ]);
  ...
  function splitStream(bytes, count) {
  ...
  function encodeStegassette(entries, srcImg, steg = COLLECTION_STEG) {
  ...
  var RevealSurface = class {
    ...
    constructor(img, opts, { className = "stegassette-player" } = {}) {
      ...
      // Channel-plan encodes only wrote SOME channels; the others are the
      // original photo at full resolution. Revealing must then rewrite only
      // the carried channels from the base, never the whole pixel — the
      // alpha-clear shortcut would swap pristine channels for an upscale.
      const chCombine = [null, null, null];
      const slots = opts.plan?.slots ?? [0, 1, 2].map((c) => ({ ch: c, combine: opts.combine }));
      for (const s of slots) chCombine[s.ch] = s.combine;
      this.carried = chCombine.map((c) => c != null);
      this.partial = this.carried.includes(false);
      this.basePx = this.partial ? baseCtx.getImageData(0, 0, W, H).data : null;
      ...
      // Large surfaces are shown through a display-resolution copy built by
      // stepped halving (a true box filter). CSS-scaling the full-res canvas
      // lets the sampler alias the per-pixel noise into zoom-dependent moire.
     ...
     */
    /** Reveal one pixel: full encodes swap the whole pixel to the base via
     * alpha, channel-plan encodes rewrite only the carried channels so the
     * untouched channels keep the full-resolution original. */
    clearPx(x, y) {
      const o = (y * this.width + x) * 4;
      if (this.partial) {
        for (let c = 0; c < 3; c++) if (this.carried[c]) this.px[o + c] = this.basePx[o + c];
      } else {
        this.px[o + 3] = 0;
      }
      this.touch(x, y);
    ...
    clearAt(pathIndex) {
      ...
      const [klx, kly] = KEYMAP[opts.keymap](
        lx,
        ly,
        IW,
        IH,
