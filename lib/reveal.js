...
/* ============================================================
   ...
   resolving the picture in the stegassette's own traversal order.

   Reveal.build(img, opts, entry) → { el, reset, seek, timer }
     img   Stegassette.Img of the encoded stegassette
     opts  the decode opts from Stegassette.decodeContainer
     entry the entry whose payload drives the sweep; an ARRAY of entries
           for a whole-stegassette sweep in byte order, where each audio
           entry develops in its own PCM read order (channel-aware) and
           everything else — text, images, the entry table, slack — in
           traversal order; or null for a plain traversal-order sweep
...
const Reveal = (() => {
  ...
  function build(img, opts, entry) {
    ...
    // `keymap`, not `keyMap` — the decoded opts come from the package now, and
    // the old spelling would fall through to the `adjacent` default here
    // without erroring, developing the image against the wrong key pixels.
    const km = Stegassette.KEYMAP[opts.keymap] || Stegassette.KEYMAP.adjacent;
    ...
    const lenOf = (e) => (e.byteLength != null ? e.byteLength : e.data.length);
    // raw PCM frames don't map to pixels in order under planar or
    // interleaved layouts — this is the order they light up in
    const pcmOrder = (e, pathLen) => {
      const f = Stegassette.parseAudioMime(e.mimetype);
      return Stegassette.computeRevealOrder({
        pathLen,
        channels: f.channels,
        bits: f.bits,
        layout: f.layout,
    ...
    const isPcm = (e) => /^audio\/l/i.test(e.mimetype || "");
    ...
    if (Array.isArray(entry)) {
      // whole-stegassette sweep: every pixel participates, and each entry's
      // span develops in the order its bytes are actually read — audio
      // channel-aware, everything else (and the entry table, gaps between
      // payloads, tail slack) in traversal order. Segments concatenate in
      // byte order, so the sweep looks like the decoder reading the
      // stegassette front to back. Adjacent entries can share a boundary
      // pixel; clearing one twice is a no-op, so the duplicate stays.
      const spans = entry
        .filter((e) => e.dataOffset != null)
        .map((e) => ({
          e,
          s: Math.floor(e.dataOffset / bpp),
          t: Math.min(pathIdx.length, Math.ceil((e.dataOffset + lenOf(e)) / bpp)),
        }))
        .sort((a, b) => a.s - b.s);
      const out = [];
      let pos = 0;
      for (const { e, s, t } of spans) {
        for (let i = pos; i < s; i++) out.push(i);
        if (isPcm(e)) {
          const sub = pcmOrder(e, Math.max(1, t - s));
          for (let i = 0; i < sub.length; i++) out.push(s + sub[i]);
        } else {
          for (let i = s; i < t; i++) out.push(i);
        }
        pos = Math.max(pos, t);
      }
      for (let i = pos; i < pathIdx.length; i++) out.push(i);
      order = Int32Array.from(out);
    } else if (entry) {
      startPx = Math.floor(entry.dataOffset / bpp);
      endPx = Math.min(
        pathIdx.length,
        Math.ceil((entry.dataOffset + lenOf(entry)) / bpp),
      );
      if (isPcm(entry)) order = pcmOrder(entry, Math.max(1, endPx - startPx));
    }
    const span = Math.max(1, order ? order.length : endPx - startPx);
