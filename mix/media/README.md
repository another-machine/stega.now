...
The demo set: loop stegassettes that the mix tab offers as **try the demo
set**.
...
These are ordinary loop stegassettes. The `make` tab makes them, and the
plain stega-now player plays each one on its own.
...
- **one with `mode: none`** — drums, or anything with no tonic — to show a
  loop that the mix never transposes
